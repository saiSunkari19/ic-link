...
func testnetCmd(ctx *server.Context, cdc *codec.Codec,
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a icd testnet",
		Long: `testnet will create "v" number of directories and populate each with
necessary files (private validator, genesis, config, etc.).
Note, strict routability for addresses is turned off in the config file.
Example:
	icd testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "icd",
		"Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "iccli",
	...
	cmd.Flags().String(
		server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", internal.DefaultStakeDenom),
		"Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001stake)")
	cmd.Flags().String(flags.FlagKeyringBackend, flags.DefaultKeyringBackend, "Select keyring's backend (os|file|test)")
	cmd.Flags().String(flagStakeDenom, internal.DefaultStakeDenom, "app's stake denom")
	return cmd
}
...
func InitTestnet(
	...
	icConfig := srvconfig.DefaultConfig()
	icConfig.MinGasPrices = minGasPrices
	...
	for i := 0; i < numValidators; i++ {
		...
		coins := sdk.Coins{
			...
			sdk.NewCoin(internal.DefaultStakeDenom, accStakingTokens),
		}
		...
		msg := staking.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(internal.DefaultStakeDenom, valTokens),
		...
		icConfigFilePath := filepath.Join(nodeDir, "config/icd.toml")
		srvconfig.WriteConfigFile(icConfigFilePath, icConfig)
	}
